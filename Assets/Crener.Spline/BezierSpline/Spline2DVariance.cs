...
namespace Crener.Spline.BezierSpline
{
    ...
    /// </summary>
    public class Spline2DVariance : MonoBehaviour, ISimpleSpline2DVarianceEditor
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private float2 CubicBezierPoint(float t, float2 p0, float2 p1, float2 p2, float2 p3)
        {
            t = Mathf.Clamp01(t);
...
#if UNITY_EDITOR
        public float2 GetPoint(float t, int index, half variance)
        {
            return CubicBezierPoint(t, index, (index + 1) % ControlPointCount, variance);
        }
        ...
        public Spline2DVarianceData? SplineEntityData { get; private set; }
        ...
        public Spline2DVarianceData ConvertSplineData()
        {
            ...
            SplineEntityData = new Spline2DVarianceData
            {
                Length = length,
                Time = time,
                Points = points,
